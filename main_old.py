...
try:
    ...
    try:
        ...
        server_socket.bind(('0.0.0.0', conf.get('ServicePort')))
        server_socket.setblocking(False)  # Неблокирующий режим
        logger.debug(f"Сокет открыт на порту: {conf.get('ServicePort')}")
    except (OSError, OverflowError):
        # Если порт занят, то проверяем кем
        errors_server.find_process_using_port(conf.get('ServicePort'))
        exit(1)
    ...
    while True:
        ...
        # настраиваем яркость
    impuls.set_bright(server_socket, conf, sended_packets)
        # настраиваем время
    if conf.get('TimeSync', 0) == 1:
        impuls.time_set(server_socket, conf, sended_packets)




    # Устанавливаем порядковые номера очереди на табло
    for number_display in range(conf.get('NumberRows')):
        if last_pid == 255:
            last_pid = 0
        last_pid += 1
        text = " "
